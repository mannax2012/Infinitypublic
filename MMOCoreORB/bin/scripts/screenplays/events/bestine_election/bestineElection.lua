BestineElection = ScreenPlay:new {
	...
	electionDuration = 14 * 24 * 60 * 60 * 1000, -- Duration of the election phase (14 days)
	timeTilNextElection = 14 * 24 * 60 * 60 * 1000, -- Time until next election, starts immediately after an election ends (14 days)
...
function BestineElection:forceReset(pTime)
	self:setCurrentPhase(1)
	self:createNewVoterList()
	if(pTime == 0) then
		rescheduleServerEvent("ElectionPhaseChange", self.electionDuration)
	else
		rescheduleServerEvent("ElectionPhaseChange", 1000)
	end
	return 0
end
...
function BestineElection:giveCampaignReward(pPlayer, candidate)
	...
	big_game_hunter:contentXP(pPlayer, 7500)
...
function BestineElection:setCurrentPhase(newPhase)
	if newPhase == 1 then
		setQuestStatus("bestineElection:currentPhase", 1)
	else
		setQuestStatus("bestineElection:currentPhase", 2)
	end
end
