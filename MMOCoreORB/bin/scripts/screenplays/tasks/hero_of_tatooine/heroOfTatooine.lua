...
HeroOfTatooineScreenPlay = ScreenPlay:new {
		...
		intellectResetTimer = 3 * 60 * 60 * 1000, -- resets after 3 hours
...
function HeroOfTatooineScreenPlay:start()
	if (isZoneEnabled("tatooine")) then
		...
		local pAltCave = getSceneObject(9995371)--(5995564)
...
function HeroOfTatooineScreenPlay:handleSuiImplication(pPlayer, pSui, eventIndex, arg0)
	...
	if (liarNum == 3) then
		...
		big_game_hunter:contentXP(pPlayer, 3500)
		...
		createEvent(self.intellectResetTimer, "HeroOfTatooineScreenPlay", "resetFailedIntellect", pPlayer, "")
	end
end

function HeroOfTatooineScreenPlay:resetFailedIntellect(pPlayer)
	if pPlayer ~= nil then
		deleteData(CreatureObject(pPlayer):getObjectID() .. ":hero_of_tat:failedIntellect")
	end
	return 1
end
