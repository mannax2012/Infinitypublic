...
function PlayerTriggers:playerLoggedIn(pPlayer)
	...
	if mq:isEnabled() then
		...
		if CreatureObject(pPlayer):hasSkill("mandalorian_rank_4") then
			mandalorianquest:removeBhAttackImmunity(pPlayer)
			local nextAttack = getRandomNumber(40 * 60 * 1000, 90 * 60 * 1000)
			util:ll("Player open to Mandalorian BH attack, next attack: " .. tostring(util:getTimerText(nextAttack, 1000)), pPlayer)
			if nextAttack ~= nil then
				createEvent(nextAttack, "mandalorianquest", "mandoBhAttack", pPlayer, "")
			else
				createEvent(45 * 60 * 1000, "mandalorianquest", "mandoBhAttack", pPlayer, "")
			end
		end
