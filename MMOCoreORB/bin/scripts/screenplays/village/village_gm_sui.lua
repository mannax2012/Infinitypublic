...
function VillageGmSui.playerInfo(pPlayer, targetID)
	...
	if (tonumber(readScreenPlayData(pTarget, "KnightTrials", "completedTrials")) == 1) then
...
function VillageGmSui.manageVisibility(pPlayer, targetID)
	...
	local sui = SuiInputBox.new("VillageGmSui", "manageVisibilityCallback")
	sui.setTitle("Visibility Management")
...
function VillageGmSui:manageVisibilityCallback(pPlayer, pSui, eventIndex, args)
	...
	local newVis = tonumber(args)
	if newVis ~= nil and newVis >= 0 and newVis <=8000 then
		util:cout(pPlayer, "Visibility set to " .. tostring(newVis) .. " on " .. CreatureObject(pTarget):getFirstName(), "green")
		PlayerObject(pGhost):setVisibility(newVis)
	end
...
function VillageGmSui.frsManagement(pPlayer, targetID)
	...
	if (luaCouncil ~= councilType) then
		...
		sui.add("Reset FRS to Rank Member (Rank 0)", "resetFRS" .. targetID)
	end
...
function VillageGmSui:changeCouncilTypeCallback(pPlayer, pSui, eventIndex, args)
	...
	CreatureObject(pTarget):awardExperience("force_rank_xp", xpAmount * -1, false)

	if (councilType == JediTrials.COUNCIL_LIGHT) then
		CreatureObject(pPlayer):sendSystemMessage("Council type has been set to Light Side.")
		PlayerObject(pGhost):setFrsRank(0)
		-- setFrsRank(0) should clear all skill boxes, but surrendering just to be sure
		VillageGmSui:surrenderFRS(pTarget, "dark")
		CreatureObject(pTarget):surrenderSkill("force_title_jedi_master")
		CreatureObject(pTarget):surrenderSkill("force_title_jedi_rank_04")
		local factionStanding = PlayerObject(pGhost):getFactionStanding("rebel")
		if factionStanding < 200 then
			PlayerObject(pGhost):setFactionStanding("rebel", 200)
		end
		CreatureObject(pTarget):setFaction(FACTIONREBEL)
		CreatureObject(pTarget):setFactionStatus(2)
		...
		awardSkill(pTarget, "force_title_jedi_rank_03")
		awardSkill(pTarget, "force_rank_light_novice")
	else
		CreatureObject(pPlayer):sendSystemMessage("Council type has been set to Dark Side.")
		PlayerObject(pGhost):setFrsRank(0)
		-- setFrsRank(0) should clear all skill boxes, but surrendering just to be sure
		VillageGmSui:surrenderFRS(pTarget, "light")
		CreatureObject(pTarget):surrenderSkill("force_title_jedi_master")
		CreatureObject(pTarget):surrenderSkill("force_title_jedi_rank_04")
		local factionStanding = PlayerObject(pGhost):getFactionStanding("imperial")
		if factionStanding < 200 then
			PlayerObject(pGhost):setFactionStanding("imperial", 200)
		end
		...
		JediTrials:setJediCouncil(pTarget, councilType)
		awardSkill(pTarget, "force_title_jedi_rank_03")
		awardSkill(pTarget, "force_rank_dark_novice")
	end
...
function VillageGmSui.fixCouncilType(pPlayer, targetID)
	...
	if (luaCouncil ~= JediTrials.COUNCIL_LIGHT and luaCouncil ~= JediTrials.COUNCIL_DARK and frsCouncil ~= JediTrials.COUNCIL_LIGHT and frsCouncil ~= JediTrials.COUNCIL_DARK) then
		...
		if (CreatureObject(pTarget):getFaction() == FACTIONIMPERIAL) then
			...
			PlayerObject(pGhost):setFrsRank(0)
			-- setFrsRank(0) should clear all skill boxes, but surrendering just to be sure
			VillageGmSui:surrenderFRS(Target, "light")
			CreatureObject(pTarget):surrenderSkill("force_title_jedi_master")
			CreatureObject(pTarget):surrenderSkill("force_title_jedi_rank_04")
	...
	if (luaCouncil == JediTrials.COUNCIL_LIGHT) then
		CreatureObject(pPlayer):sendSystemMessage("Council type has been set to Light Side.")
		PlayerObject(pGhost):setJediState(4)
		CreatureObject(pTarget):surrenderSkill("force_rank_dark_novice")
		awardSkill(pTarget, "force_rank_light_novice")
	else
		CreatureObject(pPlayer):sendSystemMessage("Council type has been set to Dark Side.")
		PlayerObject(pGhost):setJediState(8)
		CreatureObject(pTarget):surrenderSkill("force_rank_light_novice")
		awardSkill(pTarget, "force_rank_dark_novice")
	end
...
function VillageGmSui.resetFRS(pPlayer, targetID)
	local pTarget = getSceneObject(targetID)
	...
	local councilType = PlayerObject(pGhost):getFrsCouncil()
	if (councilType ~= JediTrials.COUNCIL_LIGHT and councilType ~= JediTrials.COUNCIL_DARK) then
		printLuaError("Invalid council type " .. frsCouncil .. " sent to VillageGmSui:resetFRS")
		return
	end
	...
	if (councilType == JediTrials.COUNCIL_LIGHT) then
		...
		CreatureObject(pTarget):surrenderSkill("force_title_jedi_rank_04")
		awardSkill(pTarget, "force_rank_light_novice")
		CreatureObject(pPlayer):sendSystemMessage("FRS reset to Light Rank 0.")
	else
		-- setFrsRank(0) should clear all skill boxes, but surrendering just to be sure
		VillageGmSui:surrenderFRS(pTarget, "dark")
		CreatureObject(pTarget):surrenderSkill("force_title_jedi_master")
		CreatureObject(pTarget):surrenderSkill("force_title_jedi_rank_04")
		awardSkill(pTarget, "force_rank_dark_novice")
		CreatureObject(pPlayer):sendSystemMessage("FRS reset to Dark Rank 0.")
	end
end

function VillageGmSui:surrenderFRS(pTarget, council)
	if (pTarget == nil or (council ~= "light" and council ~= "dark")) then
		return
	end

	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .."_master")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_10")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_09")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_08")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_07")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_06")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_05")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_04")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_03")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_02")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_rank_01")
	CreatureObject(pTarget):surrenderSkill("force_rank_" .. council .. "_novice")
