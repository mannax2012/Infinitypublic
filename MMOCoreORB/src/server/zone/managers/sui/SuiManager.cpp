...
void SuiManager::handleCharacterBuilderSelectItem(CreatureObject* player, SuiBox* suiBox, uint32 cancel, Vector<UnicodeString>* args) {
	...
	if (node->hasChildNodes()) {
		...
		if (templatePath.indexOf(".iff") < 0) { // Non-item selections
			...
			if (templatePath == "unlearn_all_skills") {
			...
			} else if (templatePath == "reset_buffs_only") {
				if (!player->isInCombat()) {
					player->sendSystemMessage("Your buffs have been reset.");
				...
				player->sendSystemMessage("You have received 1 million credits");
			...
			} else if (templatePath == "reset_frs_xp") {
				PlayerManager* pman = zserv->getPlayerManager();
				int currentXP = ghost->getExperience("force_rank_xp");
				pman->awardExperience(player, "force_rank_xp", (-1 * currentXP));
				player->sendSystemMessage("You have reset your Force Rank Experience to zero.");
