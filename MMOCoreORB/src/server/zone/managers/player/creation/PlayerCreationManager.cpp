...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
	...
	if (client->getCharacterCount(zoneServer.get()->getGalaxyID()) >= 7) {
		ErrorMessage* errMsg = new ErrorMessage("Notice", "Infinity allows the creation of 7 characters per account.", 0x0);
	...
	String playerLast = playerCreature->getLastName();
	if (playerLast != "")
		playerName += " " + playerLast;
	StringBuffer zBroadcast;
	zBroadcast << "[New Character Alert] " << "\\#0D9DDD" << playerName << "\\#FFFFFF has arrived. Welcome to Infinity!";
