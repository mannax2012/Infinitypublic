...
void MissionManagerImplementation::updatePlayerBountyReward(uint64 targetId, int reward) {
	...
	if (playerBountyList.contains(targetId)) {
		...
		if (reward > 2500000)
			error("Bounty exceeded 2,500,000 credits for player ID " + String::valueOf(targetId));
	}
}
...
bool MissionManagerImplementation::isBountyValidForPlayer(CreatureObject* player, PlayerBounty* bounty) {
	...
	if (zone != nullptr) {
		...
		if (zone->getZoneName() == "naboo") {   //Check to see if target is unattackable in Highgarden neutral zone

			float rangesq = 100.0f * 100.0f;
			float deltaX = -5031.0f - targetX;
			float deltaY =  2887.0f - targetY;
