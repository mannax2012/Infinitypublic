		...
		if (attackerCreature != nullptr) {
			...
			if (ghost != nullptr) {
				...
				if (defenderObject->isCreatureObject()) {
					...
					if (defenderCreo != nullptr && newBhTef && attackerCreature->hasSkill("combat_bountyhunter_investigation_03")) {   //Let's do a double-check on new BH TEFs and notify target of PVP status
...
int CombatManager::getArmorReduction(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, float damage, int hitLocation, const CreatureAttackData& data) {
	...
	if (psg != nullptr && !psg->isVulnerable(damageType)) {
		...
		if (dmgAbsorbed > 0) {
			...
			if (defender->checkCooldownRecovery("psg_damaged")) {
				...
				if (attacker->isPlayerCreature()) {
					if (defenderGhost != nullptr && defenderGhost->hasBhTef() && defender->hasSkill("combat_bountyhunter_investigation_03") && (weapon->isJediWeapon() || data.isForceAttack())) {
	...
	if (armor != nullptr && !armor->isVulnerable(damageType)) {
		...
		if (data.isForceAttack() && attacker->isPlayerCreature()) {  //Cap on resists versus player force powers in PVP
			...
			if (attackerCreature != nullptr && defender->isPlayerCreature() && defender->hasSkill("combat_bountyhunter_investigation_03") && attackerCreature->hasSkill("force_discipline_powers_master")) {
		...
		if (damageType == SharedWeaponObjectTemplate::LIGHTSABER && defender->isPlayerCreature()) {
			armorReduction = Math::min(armorReduction, 38.0f);   //Hardcap for Mando LS resist
			error("Player " + defender->getFirstName() + " is wearing lightsaber resist armor over 38%");
		}
...
float CombatManager::calculateDamage(CreatureObject* attacker, WeaponObject* weapon, CreatureObject* defender, const CreatureAttackData& data) {
	...
	if (data.isForceAttack()) {
		...
		else if (attacker->isPlayerCreature() && defender->isPlayerCreature() && commandName == "mindblast2") { //PVP reduction for mindblast2
			damage *= .85;
		}
