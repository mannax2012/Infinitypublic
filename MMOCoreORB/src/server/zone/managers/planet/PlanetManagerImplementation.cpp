...
void PlanetManagerImplementation::loadClientRegions(LuaObject* outposts) {
	...
	for (int i = 0; i < dtiff.getTotalRows(); ++i) {
		...
		areaShape->setRadius(radius + 100);  //100 meter NBZ Zone outside client region
		areaShape->setAreaCenter(x, y);
		noBuild->setAreaShape(areaShape);
		noBuild->setRadius(radius + 100);  	//100 meter NBZ Zone outside client region
...
bool PlanetManagerImplementation::isBuildingPermittedAt(float x, float y, SceneObject* object, float margin, bool checkFootprint) {
	...
	for (int i = 0; i < activeAreas.size(); ++i) {
		...
		if (area != nullptr && area->isNoBuildArea()) {
			/*
			AreaShape* areaShape = area->getAreaShape();
			if (areaShape != nullptr) {
			float x = areaShape->getAreaCenter().getX();
			float y = areaShape->getAreaCenter().getY();
			}
			error("PlanetManager - area with x = " + String::valueOf(x)  + ", y = " + String::valueOf(y)  + " is a no build area.");
			*/
			return false;
		}
	}
	...
	if (isInObjectsNoBuildZone(x, y, margin, checkFootprint)) {
		//error("PlanetManager - Is in object's no build zone.");
		return false;
	}
	...
	if (isInRangeWithPoi(x, y, 150)) {
		//error("PlanetManager - Is within 150 meters of POI.");
		return false;
	}
