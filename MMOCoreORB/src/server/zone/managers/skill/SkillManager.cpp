...
bool SkillManager::awardSkill(const String& skillName, CreatureObject* creature, bool notifyClient, bool awardRequiredSkills, bool noXpRequired, bool ignoreRequirements, bool setRank) {
	...
	if (skillName.indexOf("force_rank_") != -1){ //we are granting an FRS skill box
		...
		awardForceFromSkills(creature);  // Update force pool / regen
	}
...
bool SkillManager::surrenderSkill(const String& skillName, CreatureObject* creature, bool notifyClient, bool ignoreRequirements, bool setRank) {
	...
	if (ghost != nullptr) {
		...
		/// Update Force Power Max and Check for FRS skills
		if (skillName.beginsWith("force_")) {
			if (skillName.beginsWith("force_rank") && !skillName.contains("novice") && setRank) {
				FrsManager* frsManager = creature->getZoneServer()->getFrsManager();
				if (frsManager != nullptr) {
					int rank = frsManager->getRankForSkill(skillName);
					if (rank >= 1) {
						Locker locker(frsManager);
						Locker clocker(creature);
						frsManager->demotePlayer(creature);
						clocker.release();
						locker.release();
					}
				}
	...
	if (skillName.beginsWith("outdoors_creaturehandler")) { //Need to store active creature pets for creature handlers dropping skills
		...
		if (ghost != nullptr) {
			...
			for (int i = 0; i < ghost->getActivePetsSize(); i++) {
				...
				if (pet != nullptr) {
					...
					if (controlDevice !=nullptr && controlDevice->getPetType() == PetManager::CREATUREPET) {
						petsToStore.add(pet.castTo<CreatureObject*>());
					}
				}
			}
		
			if (petsToStore.size() > 0) {
				creature->sendSystemMessage("You have surrendered a creature handler skill, so all active creature pets have been stored.");
...
void SkillManager::surrenderAllSkills(CreatureObject* creature, bool notifyClient, bool removeForceProgression) {
	...
	if (surrenderedCH && ghost != nullptr) {
