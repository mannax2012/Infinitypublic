...
void DirectorManager::initializeLuaEngine(Lua* luaEngine) {
	...
	lua_register(luaEngine->getLuaState(), "spawnActiveAreaRect", spawnActiveAreaRect);
	...
	luaEngine->setGlobalInt("PVPTEFACTIVATED", ObserverEventType::PVPTEFACTIVATED);
	...
	luaEngine->setGlobalInt("POISON", BuffAttribute::POISON);
	luaEngine->setGlobalInt("DISEASE", BuffAttribute::DISEASE);
	...
	luaEngine->setGlobalInt("JEDI_CLOAKOFPAIN", BuffCRC::JEDI_CLOAKOFPAIN);
	luaEngine->setGlobalInt("JEDI_CORTOSIS", BuffCRC::JEDI_CORTOSIS);
...
Vector3 DirectorManager::generateSpawnPoint(String zoneName, float x, float y, float minimumDistance, 
											float maximumDistance, float extraNoBuildRadius, float sphereCollision, bool forceSpawn, bool collisionOnly) {
	bool found = false;
	...
	while (!found && retries > 0) {
		...
		found = forceSpawn || ((collisionOnly || zone->getPlanetManager()->isSpawningPermittedAt(position.getX(), position.getY(), extraNoBuildRadius)) && !CollisionManager::checkSphereCollision(position, sphereCollision, zone));
...
int DirectorManager::getSpawnPoint(lua_State* L) {
	...
	if (numberOfArguments == 5) {
		...
		collisionOnly = lua_toboolean(L, -1);
		forceSpawn = lua_toboolean(L, -2);
	...
	while (!found && retries > 0) {
		position = generateSpawnPoint(zoneName, x, y, minimumDistance, maximumDistance, 5.0, 20, false, collisionOnly);
	...
	if (!found && forceSpawn)
		position = generateSpawnPoint(zoneName, x, y, minimumDistance, maximumDistance, 5.0, 20, true, collisionOnly);
...
int DirectorManager::spawnActiveAreaRect(lua_State* L) {
	int numberOfArguments = lua_gettop(L);
	if (numberOfArguments != 8) {
		String err = "incorrect number of arguments passed to DirectorManager::spawnActiveAreaRect";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	if (object != nullptr && object->isActiveArea()) {
		...
		areaShape->setDimensions(height, width);
