...
int StructureManager::declareResidence(CreatureObject* player, StructureObject* structureObject, bool isCityHall) {
	...
	if (!isCityHall && !player->checkCooldownRecovery("declare_residence") && !ghost->isPrivileged()) {
		...
		float timeLeft = round(fabs(timeremaining->miliDifference() / 1000.0f / 3600.0f * 100.0f)) / 100.f;   // Hours to two decimal places
		player->sendSystemMessage("You cannot change residence for " + String::valueOf(timeLeft) + " hour" + ((timeLeft == 1.0f) ? "." : "s."));
		return 1;
	}
	...
	player->addCooldown("declare_residence", 4 * 3600 * 1000); //4 hours
...
void StructureManager::withdrawMaintenance(StructureObject* structure, CreatureObject* creature, int amount) {
	...
	Zone* structureZone = structure->getZone();

	String zoneName = "error";
	if (structureZone != nullptr)
		zoneName = structureZone->getZoneName();
	zoneName[0] = toupper(zoneName[0]);

	String pX = String::valueOf(ceil(structure->getWorldPositionX()));
	String pZ = String::valueOf(ceil(structure->getWorldPositionZ()));
	String pY = String::valueOf(ceil(structure->getWorldPositionY()));

	if (amount > 99999)
		error("CreditTracking: " + creature->getFirstName() + " withdrew " + String::valueOf(amount) + " credits in maintenance from a structure.  Zone: " + zoneName + 
