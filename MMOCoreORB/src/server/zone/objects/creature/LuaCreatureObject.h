...
namespace creature {
	...
	public:
		...
		int getBuffAmount(lua_State* L);
		int getBuffDuration(lua_State* L);
		int getBuffTimeLeft(lua_State* L);
		int getPlayerBountyTarget(lua_State* L);
		int awardExperienceExact(lua_State* L);
		int areInDuel(lua_State* L);
		int addWounds(lua_State* L);
		int addShockWounds(lua_State* L);
		int docBuff(lua_State* L);
		int getGuildID(lua_State* L);
