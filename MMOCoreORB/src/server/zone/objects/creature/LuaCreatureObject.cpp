...
Luna<LuaCreatureObject>::RegType LuaCreatureObject::Register[] = {
		...
		{ "awardExperienceExact", &LuaCreatureObject::awardExperienceExact },
	...
	if (realObject != nullptr) {
		...
		if (realObject->isPlayerCreature()) {
			...
			if (playerManager != nullptr) {
				level = playerManager->calculatePlayerLevel(realObject);
			}
		}
		else {
			level = realObject->getLevel();
		}
...
int LuaCreatureObject::isInCombat(lua_State* L) {
	...
	if (realObject != nullptr) {
		retVal = realObject->isInCombat();
	}
	else {
		Logger::console.error("LuaCreatureObject::isInCombat - realObject is nullptr");
	}
...
int LuaCreatureObject::awardExperienceExact(lua_State* L) {
	String experienceType = lua_tostring(L, -3);
	int experienceAmount = lua_tointeger(L, -2);
	bool sendSysMessage = lua_toboolean(L, -1);

	PlayerManager* playerManager = realObject->getZoneServer()->getPlayerManager();
	playerManager->awardExperienceExact(realObject, experienceType, experienceAmount, sendSysMessage);
...
int LuaCreatureObject::docBuff(lua_State* L) {
		...
		CreatureObject* enhancer = (CreatureObject*) lua_touserdata(L, -2);
		uint8 attribute = lua_tointeger(L, -1);
		...
		int result = realObject->docBuff(enhancer, attribute);
