...
int CreatureObjectImplementation::docBuff(CreatureObject* enhancer, uint16 attribute) {

	int GENERALERROR = -1;
	int MEDICINENOTFOUND = -2;
	int DOCLOWMIND = -3;
	int DOCINVALIDSTATE = -4;
	int PATIENTINVALIDSTATE = -5;
	int INVALIDTARGET = -6;
	int TOOFAR = -7;
	int EXISTINGBUFFHIGHER = -8;

	CreatureObject* patient = asCreatureObject();
	...
	if (!enhancer->hasSkill("science_doctor_wound_02"))
		return GENERALERROR;

	if (enhancer->hasAttackDelay() || !enhancer->checkPostureChangeDelay() || enhancer->isProne() || enhancer->isMeditating() || enhancer->isSwimming() || enhancer->isRidingMount())
		return DOCINVALIDSTATE;

	if (enhancer->isDead() || enhancer->isIncapacitated())
		return DOCINVALIDSTATE;

	if (enhancer->isInCombat()) {
		enhancer->sendSystemMessage("You cannot HealEnhance while in Combat.");
		return DOCINVALIDSTATE;
	}

	if (enhancer->isDancing() || enhancer->isPlayingMusic()) {
		enhancer->sendSystemMessage("You cannot HealEnhance while dancing or playing music.");
		return DOCINVALIDSTATE;
	}

	if (enhancer->getWorldPosition().distanceTo(patient->getWorldPosition()) - enhancer->getTemplateRadius() - patient->getTemplateRadius() > 25.0f) {
		enhancer->sendSystemMessage("You must be within 25 meters of the patient to properly supervise your assistant.");
		return TOOFAR;
	}
	...
	if (patient->isDead()) {
		enhancer->sendSystemMessage("You cannot HealEnhance the dead.");
		return PATIENTINVALIDSTATE;
	}

	if (patient->isInCombat()) {
		enhancer->sendSystemMessage("You cannot HealEnhance your target while they are in Combat.");
		return PATIENTINVALIDSTATE;
	}

	Locker locker(enhancer);
	...
	if (inventory != nullptr) {
		for (int i = 0; i < inventory->getContainerObjectsSize(); ++i) {
			...
			if (pharma->isEnhancePack()) {
				EnhancePack* enhancePackTemp = cast<EnhancePack*>(pharma);

				if (enhancePackTemp->getMedicineUseRequired() <= medicineUse && enhancePackTemp->getAttribute() == attribute) {
					enhancePack = enhancePackTemp;
					break;
				}
			}
		}
	...
	int mindCostNew = enhancer->calculateCostAdjustment(CreatureAttribute::FOCUS, 30);
	...
	if (!enhancer->hasBuff(BuffCRC::FOOD_DISH_BIVOLI_TEMPARI))
		enhancer->eatBivoli();

	int droidBuff = enhancer->getSkillModOfType("private_medical_rating",SkillModManager::DROID);	
	enhancer->removeAllSkillModsOfType(SkillModManager::DROID,true);
	enhancer->addSkillMod(SkillModManager::DROID,"private_medical_rating",110,true);
	
	//Applies battle fatigue, checks wound treatment, medical rating, etc.
	uint16 buffPower = getEnhancePackStrength(enhancePack, enhancer, patient);

	enhancer->removeAllSkillModsOfType(SkillModManager::DROID,true);
	enhancer->addSkillMod(SkillModManager::DROID,"private_medical_rating",droidBuff,true);

	if (buffPower < currentBuff) {
		return EXISTINGBUFFHIGHER;
	}

	int16 amountEnhanced = 0;
	...
	if (enhancer->isPlayerCreature() && patient->isPlayerCreature()) {
		playerManager->sendBattleFatigueMessage(enhancer, patient);
	}
...
void CreatureObjectImplementation::sendEnhanceMessage(CreatureObject* creature, CreatureObject* target, uint16 attribute, uint32 buffApplied) {
	if (!creature->isPlayerCreature())
		return;
...
uint32 CreatureObjectImplementation::getEnhancePackStrength(EnhancePack* enhancePack, CreatureObject* enhancer, CreatureObject* patient) {
	uint32 buffPower = 0;
...
uint32 CreatureObjectImplementation::getBuffStrength(Buff* existingbuff, uint16 attribute) {
...
void CreatureObjectImplementation::awardXp(CreatureObject* creature, const String& type, int power) {
	if (!creature->isPlayerCreature())
		return;
...
void CreatureObjectImplementation::eatBivoli() {

	CreatureObject* enhancer = asCreatureObject();
	...
	ManagedReference<Consumable*> bivoli = nullptr;

	for (int i = 0; i < inventory->getContainerObjectsSize(); ++i) {
		ManagedReference<SceneObject*> sceno = inventory->getContainerObject(i);
		if (sceno != nullptr && sceno->getGameObjectType() == SceneObjectType::FOOD) {
			ManagedReference<Consumable*> consumable = cast<Consumable*>(sceno.get());
			if (consumable == nullptr)
				continue;
			String templateName = sceno->getObjectTemplate()->getTemplateFileName();
			if (templateName == "dish_bivoli_tempari") {
				bivoli = consumable;
				break;
			}
		}
	...
	PlayerObject* ghost = enhancer->getPlayerObject();

	if (ghost == nullptr)
		return;

	int availFill = ghost->getFoodFillingMax() - ghost->getFoodFilling();
	int bivoliFill = bivoli->getFilling();

	if (bivoliFill > availFill) {
		enhancer->sendSystemMessage("@error_message:full_food"); //You are too full to eat that
		return;
	}

	Locker locker(bivoli);

	ManagedReference<Buff*> buff = nullptr;

	buff = new Buff(enhancer, BuffCRC::FOOD_DISH_BIVOLI_TEMPARI, bivoli->getDuration(), BuffType::FOOD);
	...
	buff->setSkillModifier("healing_wound_treatment", bivoli->getNutrition());

	enhancer->addBuff(buff);
	...
	params.setTO("@stat_n:healing_wound_treatment");
	enhancer->sendSystemMessage(params); // According to evidence, this should send before the consumed message, so we don't want to set the buff spam start.

	ghost->setFoodFilling(ghost->getFoodFilling() + bivoliFill, true);

	StringIdChatParameter stringId("base_player", "prose_consume_item"); // You consume %TT.
	stringId.setTT(bivoli->getObjectID());
	enhancer->sendSystemMessage(stringId);

	//Consume a charge from the item, destroy it if it reaches 0 charges remaining.
	bivoli->decreaseUseCount();
}