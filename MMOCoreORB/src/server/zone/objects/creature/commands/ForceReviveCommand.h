...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (playerFactionBase && !creatureTarget->checkCooldownRecovery("forceRevive"))  {
			Time* cooldownTime = creatureTarget->getCooldownTime("forceRevive");
			if (cooldownTime != nullptr) {
				float timeLeft = round(fabs(cooldownTime->miliDifference() / 1000.f));
				creature->sendSystemMessage("You can use the forceRevive command again on this target in " + String::valueOf(timeLeft) + " second" + ((timeLeft == 1.0f) ? "." : "s."));
				return GENERALERROR;
			}
			else {
