...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		//non ch doesn't seem to have keep_creature, effectively returns 0/
		...
		//Non CH can only have 1 active pet.
		if(targetPlayer->hasSkill("outdoors_creaturehandler_novice") && ((petLevel + pet->getLevel()) > Math::max(targetPlayer->getSkillMod("tame_level"),10)))
		{
