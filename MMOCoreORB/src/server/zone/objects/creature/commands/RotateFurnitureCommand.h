	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!(ghost->getAdminLevel() == 15))  { //// Restrictions for non-admin characters
			...
			if (data != nullptr) {
				...
				DestructibleCityDecorationMenuComponent* destructibleDecorationMenu = cast<DestructibleCityDecorationMenuComponent*>(obj->getObjectMenuComponent());

				if (decorationMenu == nullptr && destructibleDecorationMenu == nullptr && !obj->isMissionTerminal() && !obj->getObjectTemplate()->getFullTemplateString().contains("trainer")) {
