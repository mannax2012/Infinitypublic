...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (objectId != 0) {  //Enhance used from item or from toolbar
			SceneObject* inventory = creature->getSlottedObject("inventory");
			SceneObject* backpack = creature->getSlottedObject("back");
			...
			if (enhancePack == nullptr) { //Third, check for containers in inventory or backpack
				SceneObject* medicalPack = creature->getZoneServer()->getObject(objectId);
