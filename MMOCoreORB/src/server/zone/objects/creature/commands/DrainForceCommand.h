...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		/*
		float targetCooldown = 3.5f;	// Target cooldown for Jedi master healers

		if (!targetCreature->checkCooldownRecovery("drainforcetarget")) {
			creature->sendSystemMessage("Jedi master healers can only be drained once every " + String::valueOf(targetCooldown) + " seconds.");
			return GENERALERROR;
        }
		*/
		...
		if (manager->startCombat(creature, targetCreature, false)) { //lockDefender = false because already locked above.
			...
			/*
			if (targetCreature->hasSkill("force_discipline_healing_master")) {// Cooldown on draining master force healers
				targetCreature->updateCooldownTimer("drainforcetarget", targetCooldown * 1000);
			}
			*/
