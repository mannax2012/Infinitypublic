...
void TangibleObjectImplementation::setPvpStatusBitmask(uint32 bitmask, bool notifyClient) {
	...
	if (isPlayerCreature()) {
		...
		Locker locker(asTangibleObject());
	...
	if (isImperial() && !(object->isRebel())) {
		...
		if (ai->isPet()) {
			...
			if (pcd != NULL && pcd->getPetType() == PetManager::FACTIONPET) {
				...
				if ((object->isPlayerCreature() || object->isPet()) && (object->getFactionStatus() != FactionStatus::OVERT || getFactionStatus() != FactionStatus::OVERT))  {
					return false;
				}
