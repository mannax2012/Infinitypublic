...
float ArmorObjectImplementation::getTypeValue(int type, float value) {
	...
	if (type == SharedWeaponObjectTemplate::LIGHTSABER && !isPsgArmorObject()) {
		if (newValue > 38) {
				//error("Someone using armor with over 38% lightsaber resist." + String::valueOf(newValue) + "%.  Armor capped at 38%.");
				newValue = 38;  //Hardcap for Mando LS Resist until better resources avaiable
		}
	}
