...
void PetControlDeviceImplementation::callObject(CreatureObject* player) {
	...
	if (player->isInCombat() && (!player->hasSkill("outdoors_creaturehandler_master") || petType != PetManager::CREATUREPET)) {
		player->sendSystemMessage("@pet/pet_menu:cant_call_combat"); // You cannot call this pet while in combat.
		return;
	}
	...
	*/
	...
	if (petType == PetManager::CREATUREPET) {
		...
		if (ch) {
			...
			maxLevelofPets = Math::max(player->getSkillMod("tame_level"),10);
		}
	...
	for (int i = 0; i < ghost->getActivePetsSize(); ++i) {
		...
		if (object != nullptr) {
			...
			if (object->isCreature() && petType == PetManager::CREATUREPET) {
				...
				if (activePetTemplate == nullptr || callingPetTemplate == nullptr || (activePetTemplate->getTemplateName() != "at_st" && activePetTemplate->getTemplateName() != "rebel_droideka"))
					continue;
	...
	if (player->getCurrentCamp() == nullptr && player->getCityRegion() == nullptr) {
		...
		if (!player->hasSkill("outdoors_creaturehandler_master") || petType != PetManager::CREATUREPET) { // Cancel pet call on combat start unless Master CH calling creature pet
...
bool PetControlDeviceImplementation::growPet(CreatureObject* player, bool force, bool adult) {
	...
	int stagesToGrow = timeDelta / 8640; // 8640 per stage for live (24 hour growth)
...
bool PetControlDeviceImplementation::canBeTradedTo(CreatureObject* player, CreatureObject* receiver, int numberInTrade) {
	...
	if (petType == PetManager::FACTIONPET) {
		...
		if (pet != nullptr) {
			String objectString = pet->getControlDeviceTemplate();
			if (objectString != "") {
				ManagedReference<PlayerObject*> ghost = player->getPlayerObject();
				bool isAdmin = false;
				if (ghost != nullptr)
					isAdmin = ghost->isAdmin();
				if (objectString.contains("belsavis_varactyl") && !isAdmin) {    // Admin's can trade as an exception
					player->sendSystemMessage("Belsavis varactyl pets cannot be traded.  The Black Krrsantans would kill everyone involved!  Transfer failed."); 
					return false;
				}
			}
		}
		...
		if (ch) {
			...
			maxLevelofPets = Math::max(receiver->getSkillMod("tame_level"),10);
		}
