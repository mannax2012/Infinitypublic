...
Luna<LuaBuildingObject>::RegType LuaBuildingObject::Register[] = {
		...
		{ "ejectObject", &LuaBuildingObject::ejectObject},
		{ 0, 0 }
};
...
int LuaBuildingObject::getTotalCellNumber(lua_State* L) {
	lua_pushinteger(L, realObject->getTotalCellNumber());
...
int LuaBuildingObject::ejectObject(lua_State* L) {

	CreatureObject* creature = (CreatureObject*)lua_touserdata(L, -1);

	if (realObject != nullptr && creature != nullptr && creature->isCreatureObject())

		realObject->ejectObject(creature);
