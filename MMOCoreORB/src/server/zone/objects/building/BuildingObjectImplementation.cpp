...
bool BuildingObjectImplementation::isAllowedEntry(CreatureObject* player) {
	...
	CreatureObject* creo = player;

	if (player != nullptr && player->isPet()) {
	...
	if (conComp != nullptr) {
		...
		return checkContainerPermission(creo,ContainerPermissions::WALKIN);
	}
	...
	if (!isClientObject()) {
		PlayerObject* ghost = creo->getPlayerObject().get();
	...
	if (getOwnerObjectID() == creo->getObjectID())
		return true;

	if (isOnBanList(creo))
		return false;

	if (isPrivateStructure() && !isOnEntryList(creo))
		return false;
		...
		if (creature->isPet()) {
			...
			if (owner != nullptr) {
				...
				if( petManager != nullptr) {
					...
					if (pet != nullptr) {
						petManager->enqueuePetCommand(owner, pet, STRING_HASHCODE("petfollow"), String::valueOf(owner->getObjectID()), true);
						ejectObject(creature);
						ejected = true;
						petManager->enqueuePetCommand(owner, pet, STRING_HASHCODE("petfollow"), String::valueOf(owner->getObjectID()), true);

						ManagedReference<PetManager*> strongPetManager = petManager;
						ManagedReference<CreatureObject*> strongOwner = owner;
						ManagedReference<AiAgent*> strongPet = pet;
								Core::getTaskManager()->scheduleTask([strongPetManager,strongOwner,strongPet] {
									if (strongPetManager != nullptr && strongOwner != nullptr && strongPet != nullptr)
										strongPetManager->enqueuePetCommand(strongOwner, strongPet, STRING_HASHCODE("petfollow"), String::valueOf(strongOwner->getObjectID()), true);
								}, "PetFollowOwner", 1000);	
					}
				}
			}
...
void BuildingObjectImplementation::onEnter(CreatureObject* creature) {
	...
	if (creature->isPlayerCreature()) {
		...
		addTemplateSkillMods(creature);

		Locker accessLock(&paidAccessListMutex);

		if (isGCWBase() && factionBaseType != GCWManager::STATICFACTIONBASE) {
			if (!checkContainerPermission(creature, ContainerPermissions::WALKIN)) {
				ejectObject(creature);
				return;
			}
		}

		if (accessFee > 0 && !isOnEntryList(creature)) {
			//thread safety issues!
			if (paidAccessList.contains(creature->getObjectID())) {
				uint32 expireTime = paidAccessList.get(creature->getObjectID());

				if(expireTime <= time(nullptr)) {
					paidAccessList.drop(creature->getObjectID());

					promptPayAccessFee(creature);
		...
		notifyObservers(ObserverEventType::ENTEREDBUILDING, creature, 0);

		//If they are inside, and aren't allowed to be, then kick them out!
		if (!isClientObject() && (!isAllowedEntry(creature) || isCondemned())) {
			ejectObject(creature);
			...
			if (isCondemned()) {
				...
				if (ownerOid == creature->getObjectID()) {
					StructureManager::instance()->promptPayUncondemnMaintenance(creature, asBuildingObject());
				} else {
		...
		if (isCivicStructure() && isCityBanned(creature)) {
			ejectObject(creature);
			creature->sendSystemMessage("@city/city:youre_city_banned"); // you are banned from this city and may not use any of its public services and structures
				...
				//If they are inside, and aren't allowed to be, then kick them out!
	} else if (creature->isPet() && !isClientObject() && (!isAllowedEntry(creature) || isCondemned())) {
...
uint32 BuildingObjectImplementation::getMaximumNumberOfPlayerItems() {
	if (isCivicStructure() )
		return 1500;     //Infinity Custom Value
	...
	return Math::min(maxItems, lots * 300);    //Infinity Custom Value
}
