		...
		if (objective->isPlayerTarget()) {
			...
			if (zoneServer != nullptr) {
				...
				if (target != nullptr && target->isPlayerCreature()) {
					...
					String droidType = arakyd ? "probe droid" : "seeker droid";

					String playerName = player->getFirstName();
					String targetName = target->getFirstName();

					if (player->getLastName() != "")
						playerName += " " + player->getLastName();

					if (target->getLastName() != "")
						targetName += " " + target->getLastName();

					Logger::console.error("FindTargetTask:  Bounty hunter " + playerName + " successfully tracked player " + targetName + " with a " + droidType);
				}
			}
		}
