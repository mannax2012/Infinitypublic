...
void PlayerObjectImplementation::notifyOnline() {
	...
	error("PlayerLogging: " + firstName + " is online.");

	PlayerManager* playerManager = server->getPlayerManager();
	SkillManager* skillManager = zoneServer->getSkillManager();

	// Infinity dev admin access for TC
	if (zoneServer->getGalaxyName() == "SWG Infinity TC" && (firstName == "bbq" || firstName == "stepo" || firstName == "lafa")) {
	...
	// Infinity tester access for TC
	if (zoneServer->getGalaxyName() == "SWG Infinity TC" && (firstName == "tk-lolo" || firstName == "kat" || firstName == "keke" || firstName == "khaizek" )) {

		if (playerManager != nullptr && skillManager != nullptr) {
			skillManager->addAbility(_this.getReferenceUnsafeStaticCast(), "admin");
			playerManager->updatePermissionLevel(playerCreature, 6);
		}
	}
	...
	while (itemsRemain && (attempts <= 5)) {
		...
		for (int i = 0; i < playerCreature->getSlottedObjectsSize(); ++i) {
			...
			if (object->isWearableObject()) {
				...
				if (object->isDestroyed()) {    // Let's remove zero condition wearables
					...
					if (inventory != nullptr){ 
						...
						error("Removing destroyed wearable " + object->getObjectName()->getStringID() + " for player " + playerCreature->getFirstName());
						continue;
					}
				}
				...
				if (tanoData != nullptr) {
					...
					if (skillsRequired.size() > 0) {   // We have a wearable item with certifications required
						...
						if (inventory != nullptr && !hasSkill){ // Remove uncertified wearable item
							...
							ObjectController* objectController = zoneServer->getObjectController();
							objectController->transferObject(object, inventory, -1, true, true); // -1 is the containmentType to unequip items
							error("Removing uncertified wearable " + object->getObjectName()->getStringID() + " for player " + playerCreature->getFirstName());
							itemsRemain  = true;
						}
					}
				}
...
void PlayerObjectImplementation::updateLastPvpCombatActionTimestamp(bool updateGcwAction, bool updateBhAction) {
	...
	if (!alreadyHasTef) {
		parent->notifyObservers(ObserverEventType::PVPTEFACTIVATED);
	...
	for (int i = 0; i < closeObjects.size(); ++i) {
		...
		if (building != nullptr) {
			...
			for (int i = 0; i < getActivePetsSize(); ++i) {
				ManagedReference<AiAgent*> pet = getActivePet(i);
				if (pet != nullptr) {
					building->updateCellPermissionsTo(pet);
				}
			}
		}
	}
